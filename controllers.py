...
class OmegaModbus:
    ...
    def __init__(self, com_port, baudrate, timeout):
        ...
        self.baudrate = baudrate
        self.timeout = timeout
        self.instrument = minimalmodbus.Instrument(self.com_port, slaveaddress=1, mode='rtu') 
        self.activate()

    def activate(self):
        try:
            # Input sensor 0 is TC, 1 is RTD, 2 is thrm, 3 is Proc
            # Make TC (thermocouple)
            stored_value = self.instrument.write_register(642, 0)
            stored_value = self.instrument.read_register(642)
            print("Input sensor:", stored_value)

            # TC type 1 is k, type 2 is J, type 3 is t, ...
            stored_value = self.instrument.write_register(643, 1)
            stored_value = self.instrument.read_register(643)
            print("TC Type:", stored_value)

            print("Omega Modbus RTU Controller ready...")
            return True
    ...
    def read(self):
        ...
        try:
            INPUT_REG = 528
            stored_value = round(self.instrument.read_float(INPUT_REG, 3), 1)
            return {
                "t1": stored_value,
            }
